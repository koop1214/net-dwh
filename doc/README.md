...
## Общее задание
Описанные выше трансформации объединяем в задание (job), чтобы можно было выполнять ETL по расписанию. Задание также гарантирует очередность запуска операций трансформаций, т.к. важно, чтобы таблицы измерений обрабатывались раньше таблицы фактов.
